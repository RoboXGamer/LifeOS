...
export function createConvexAction<ActionRef extends FunctionReference<"action">>(
  ...
  if (!client) {
    if (!isServer) throw new Error("createConvexAction must be used within ConvexProvider");
    const stub = (async () => {
      throw new Error("createConvexAction cannot execute during SSR without ConvexProvider");
