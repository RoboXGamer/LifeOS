...
export function ItemRow(props: {
  ...
  onCyclePriority?: (itemId: Id<"items">) => void;
}) {
  ...
  return (
    ...
    >
      ...
      <Show
        when={props.onCyclePriority}
        fallback={
          <Show when={item().priority}>
            {(priority) => (
              <span class={`product-priority p${priority()}`}>
                P{priority()}
              </span>
            )}
          </Show>
      ...
      >
        <button
          type="button"
          class={`product-priority p${item().priority ?? 0}`}
          aria-label={`Change priority for ${item().title}`}
          title="Cycle priority"
          onClick={() => props.onCyclePriority?.(item()._id)}
        >
          {item().priority ? `P${item().priority}` : "—"}
        </button>
      </Show>
      <Show when={item().type === "Task" && item().status}>
        {(status) => (
          <span
            class={`product-status status-${status().toLowerCase().replace(" ", "-")}`}
          >
            {status()}
          </span>
        )}
